...
@EnableKafka  // Permite a detecção de kafka ouvinte (KafkaListener) automaticamente pelo SpringBoot
@Configuration
public class KafkaConsumerConfig<K extends Serializable, V extends SpecificRecordBase> {
    ...
    // KafkaConsumerConfigData está no módulo: app-config-data
    // para ler as informações do arquivo config-client-kafka_to_elastic.yml no módulo (Github): config-server-repository
    ...
    public KafkaListenerContainerFactory<ConcurrentMessageListenerContainer<K, V>> kafkaListenerContainerFactory() {
        ...
        // Para indicar que queremos obter dados do kafka não um por um, mas como lotes
        factory.setBatchListener(kafkaConsumerConfigData.getBatchListener());
        factory.setConcurrency(kafkaConsumerConfigData.getConcurrencyLevel());
        // Indicar se queremos começar automaticamente ou depois
        factory.setAutoStartup(kafkaConsumerConfigData.getAutoStartup());
        // Determina quanto tempo esperaremos até que pelo menos um registro esteja disponível quando
        // chamamos o método poll.
